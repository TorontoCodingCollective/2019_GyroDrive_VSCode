 ...
 * speed controllers
 * <p>
 * Care should be taken to ensure that the speed controllers are properly connected and are 
 * all driving the motors in the same direction.
 */
public class TCanSpeedController extends TSpeedController {
	 ...
	 * <p>
	 * Supports any number of CAN speed controllers of the same type driving in the same direction.  The 
	 * follower devices will be configured to follow the primary CAN device.  The speed will only
	 * ever be set on the primary CAN device.
	 * <p>
	 * If there is an encoder or limit switches attached to one of the devices, it should be 
	 * set as the primary device, not the follower.  Encoders and limits cannot be retrieved
	 * from follower devices.
	 * 
	 ...
	 * CAN Speed Controller
	 * <p>
	 * Supports two CAN speed controllers of different types.  The 
	 * follower devices will be configured to follow the primary CAN device.  
	 * The speed will only ever be set on the primary CAN device.
	 * <p>
	 * If there is an encoder or limit switches attached to one of the devices, it should be 
	 * set as the primary device, not the follower.  Encoders and limits cannot be retrieved
	 * from follower devices.
	 ...
	 * <p>
	 * NOTE: There can be significant delay getting the speed from the controller
	 *       so this routine returns the last speed value sent to the controller.
	 *       The actual value on the CAN device is not returned.
	 * <p>
	 * {@inheritDoc}
	 */
	@Override
	 ...
	 * @param controllerType a valid {@link TCanSpeedControllerType}
	 * @param canAddress a valid unique CAN address
	 * @return TCanSpeedController of the correct type.  By default, the 
	 * speed controller will be a TalonSRX.
	 */
