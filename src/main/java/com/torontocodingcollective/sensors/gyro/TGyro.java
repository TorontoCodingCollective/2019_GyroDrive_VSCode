 ...
 * <p>
 * The TGyro class provides a unifying interface for all 
 * gyro types and is guaranteed to return a value 
 * of 0 <= angle < 360 degrees.
 * <p>
 * The TGyro class supports for clarity the methods of 
 * {@link Gyro} except for the {@link #free()} method.
 */
public abstract class TGyro extends GyroBase {
	 ...
	 * @param isInverted {@code true} if the gyro is 
	 * inverted {@code false} otherwise.
	 */
	 ...
	 * <p>
	 * NOTE: some gyros can only be calibrated on power up and 
	 * do not support subsequent calibration.
	 * NOTE: ensure the robot is first turned on while it's sitting at rest before the
	 * competition starts to allow for correct gyro calibration.
	 */
	 ...
	 * @param rawAngle
	 * @return normalized angle 0 <= angle < 360 where
	 * the inversion of the gyro is taken into account
	 */
	 ...
	 * <p>
	 * Not all gyros allow for multiple axis and 
	 * this routine will return 0 always if the 
	 * pitch is not supported.
	 * @return pitch in degrees or 0 if pitch is
	 * not supported
	 */
	 ...
	 * @return boolean {@code true} if the gyro is 
	 * inverted, {@code false} otherwise
	 */
	...
	/**
	 * Get the angle normalized to a value between 
	 * 0 and 360 degrees
	 * @param rawAngle value
	 ...
	 * <p>
	 * NOTE: This routine never performs a calibration
	 * of the gyro, so the amount of drift will not 
	 * change.  In order to calibrate the gyro, use the 
	 ...
	 * @return boolean {@code true} indicates that pitch
	 * is supported {@code false} indicates pitch is not 
	 * supported
	 */
