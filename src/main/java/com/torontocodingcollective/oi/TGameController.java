...
public abstract class TGameController extends Joystick {
	...
	/**
	 * The deadband used for this TGameController when getting the 
	 * value of an axis.
	 * <p>
	 * If the axis value of the stick or trigger is less
	 * than this value, then a value of zero is returned.  Most
	 * GameControllers will have a deadband of less than 0.07. 
	 * <p>
	 * Changes to this value will take place immediately.
	 * <p>
	 * In order to test the deadband of the sticks on your controller
	 * set this value to 0 to turn off the deadband filter.
	 * <p>
	 * Values greater than the deadband are returned.  Values
	 * less than or equal to the deadband are returned as zero.
	 * <p>
	 * For the default deadband of .07, values of .08 and higher will 
	 * not be filtered.
	 */
	 ...
	 * @return double value in the range 0 to 1.0 rounded to the
	 * nearest .01.
	 */
	...
	/**
	 * Get a string representing the pressed buttons on this
	 * controller
	 * @return
	 */
	 ...
	 * @param axisNumber value indicating the raw axis index in the 
	 * DriverStation.  DriverStation axis indicators start at index 0.
	 * @return axisValue rounded to 2 decimal places and filtered by
	 * the current {@link #axisDeadband}.
	 */
	 ...
	 * <p>
	 * The TStickPostion object contains the (x,y) coordinates of 
	 * the stick.  Only TStick values of {@link TStick#LEFT} or 
	 * {@link TStick#RIGHT} are suppported.
	 * @param stick {@link TStick#LEFT} or {@link TStick#RIGHT}
	 * @return TStickPosition of the stick or {@code null} if an
	 * invalid stick is specified.
	 */
	public TStickPosition getStickPosition(TStick stick) {
		switch (stick) {
		case LEFT:
		case RIGHT:
			return new TStickPosition(
					getAxis(stick, TAxis.X), getAxis(stick, TAxis.Y));
		default:
			System.out.println("Unsupported stick type " + stick +
					" for TGameController.getStickPosition()");
			return null;
		}
	}
	...
	public boolean isStickActive(TStick stick) {
		...
		if (   Math.abs(stickPosition.x) > 0
			|| Math.abs(stickPosition.y) > 0 ) {
			return true;
		}
	 ...
	 * Is User Active
	 * <p>
	 * Indicates if the user is actively pressing a button
	 * or activating a stick on the controller
	 * @return {@code true} if the user is actively using the 
	 * controller, {@code false} otherwise
	 */
	public boolean isUserActive() {
		
		// If one of the sticks is active, then the user is active
		if (   isStickActive(TStick.LEFT)
			|| isStickActive(TStick.RIGHT) ) {
			return true;
		}
		
		// If one of the triggers is pressed, then the user is active
		if (   getTrigger(TTrigger.LEFT)  > 0
			|| getTrigger(TTrigger.RIGHT) > 0 ) {
			return true;
		}
	...
	public String toString() {
		...
		sb
		.append(' ').append(getStickPosition(TStick.LEFT))
		.append(' ').append(getStickPosition(TStick.RIGHT))
		.append(" Triggers(").append(getTrigger(TTrigger.LEFT))
		.append(',').append(getTrigger(TTrigger.RIGHT)).append(')');
