 ...
 * <p>  
 * This routine will only return a POV angle on the first occurance of a 
 * after all POV buttons were released.  It will then return {@code -1} for all subsequent
 * calls to the {@link #get()} routine even if a POV button is still pressed.
 * <p>
 * All POV buttons must be released before the next press is detected. 
 * <p>
 * NOTE: This routine will not perform well when trying to get a 45 deg value since
 * it is unlikely that the underlying POV will return a 45 deg increment on the 
 * first press.  It is recommended to only use this class to detect values of
...
public class TPOVPressDetector {
	 ...
	 * <p>
	 * NOTE: This routine should only be called once per controller cycle
	 * since subsequent calls will always return {@value -1}
	 * @return {@code true} if a press was detected since the last call 
